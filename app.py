...
def preprocess_data(df):
    ...
    cols_to_zero = ['cabbage', 'radish', 'garlic',
                    'onion', 'daikon', 'cilantro', 'artichoke']
...
def plot_predictions_over_time(df, vegetables, rolling_mean_window):
    ...
    for i, veg in enumerate(vegetables):
        ax.plot(df.index, df[veg], label=veg,
                linewidth=2, color=colors[i % num_colors])
        rolling_mean = df[veg].rolling(window=rolling_mean_window).mean()
        ax.plot(df.index, rolling_mean,
                label=f'{veg} ({rolling_mean_window}-day Rolling Mean)', linestyle='--', color=colors[i % num_colors])
...
rolling_mean_window = st.sidebar.slider(
    'Rolling Mean Window', min_value=1, max_value=30, value=7)
